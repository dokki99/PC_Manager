...
int ini;																																										//커멘드에서 각 정보마다 버퍼(메세지)의 인덱스
		...
		if (nReturn == 0 || nReturn == SOCKET_ERROR) {
			...
			if (buf[0] == '1') {
				...
				if (buf[1]=='1') {					//로그인되면 남은시간처리되고 아이디저장			//로그인실패 이유: 1.아이디/비번오류,2.남은 시간 0이하
					pctimei = 0;
					for (int i = 2; i < 4; i++) {
						pctimebuf[pctimei++] = buf[i];	//갯수나 번호는 1이아닌 01형식(2자리)
					}
					pctimebuf[pctimei] = '\0';								//남은시간 담기
					...
					if (pctime >= 0) {
						...
						if (pctime > 0) {												//유저의 시간이 0 초과여야 로그인 가능
							chklogin = 1;
							chktimer = 1;
							wsprintf(buf, "%s", "로그인성공");	//로그인과 타이머 동작 시작
						}
						else wsprintf(buf, "%s", "로그인실패(잔여시간없음)");	//유저의 시간이 0이하일때
					}
				}
				else wsprintf(buf, "%s", "로그인실패");					//cmd를 40으로 받으면 실패
			}
			else if (buf[0] == '2') {
				if (buf[1] == '1') {
					if (lstrcmp(orderbuf, TEXT("211")) == 0) {			//음료상품구매처리
						wsprintf(buf, "%s", "DRINK구입성공");
					}
					else wsprintf(buf, "%s", "DRINK구입실패");			//서버로부터  "210" 받았을 때
				}
				else if (buf[1] == '2') {
					if (lstrcmp(orderbuf, TEXT("221")) == 0) {			//식품구매처리
						wsprintf(buf, "%s", "FOOD구입성공");
					}
					else wsprintf(buf, "%s", "FOOD구입실패");			//서버로부터  "220" 받았을 때
				}
				else if (buf[1] == '3') {
					if (lstrcmp(orderbuf, TEXT("231")) == 0) {			//피시상품구매처리
				...
				if (lstrcmp(buf, TEXT("71")) == 0) {
					wsprintf(buf, "%s", "탈퇴성공");							//탈퇴되도 창 닫지 말기
				}
				else wsprintf(buf, "%s", "탈퇴실패");						//서버로부터  "70" 받았을 때
			}
			if (lstrlen(buf) != 0) {//모든 메세지 나타내기
				sprintf_s(strTemp, "수신한 메시지:%s", buf);								//위에서 cmd를 변환한 메세지를 프로시저의 채팅 에디트에 띄우기
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_TIMER:
		...
		case 1:												//남은 시간 타이머
			...
			break;
		case 2:																//나가기버튼 눌렸을때/남은시간 0일때 남은시간 타이머 종료하고 다시 타이머 만들기(로그인을 계속 받기 위해)
			if (chkendbtn == 1) {
...
LRESULT CALLBACK SelSeatProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {						//자리선택
	...
	case WM_COMMAND:
		...
		case ID_B_SELSEAT:					//자리선택 눌렸을때
			...
			GetWindowText(hEdit_Sn,tgnum, sizeof(tgnum));						//자리 선택(02등 2자리 형식으로)
